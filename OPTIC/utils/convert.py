...
def convert_encoder_to_target(
    ...
    for i, layer in enumerate(layers):
        if not (start <= i < end):
            continue
        if i == 0:
            net.bn1 = convert_norm(
                net.bn1, norm, net.bn1.num_features, idx, fea_size=input_size // 2
            )
            idx += 1
        else:
            down_sample = 2 ** (1 + i)
            ...
            for j, block in enumerate(layer):
                ...
                if block.downsample is not None:
                    block.downsample[1] = convert_norm(
                        block.downsample[1],
                        norm,
                        block.downsample[1].num_features,
                        idx,
                        fea_size=input_size // down_sample,
                    )
                    idx += 1
...
def convert_decoder_to_target(
    ...
    for i, layer in enumerate(layers):
        ...
        if i == 4:
            ...
            down_sample = 2 ** (4 - i)
            layer.bn = convert_norm(
                layer.bn, norm, layer.bn.num_features, idx, input_size // down_sample
            )
            idx += 1
    return net
